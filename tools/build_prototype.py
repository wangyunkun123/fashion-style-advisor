...
function loadMonthlyReport(){{var el=document.getElementById('wrd-monthly-content');if(!__apiCache['monthly'])el.innerHTML='<div class=\"wrd-loading\">加载中...</div>';cachedFetch('monthly','/api/wardrobe/stats',function(d,fromCache){{var html='<div class=\"wrd-monthly\"><div class=\"wm-card\"><div class=\"wm-title\">📈 核心数据</div><div class=\"wm-stat-row\"><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+d.total_outfits+'</div><div class=\"wm-stat-lbl\">累计穿搭</div></div><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+d.active_days+'</div><div class=\"wm-stat-lbl\">活跃天数</div></div><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+d.avg_rating+'</div><div class=\"wm-stat-lbl\">平均评分</div></div></div><div class=\"wm-stat-row\"><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+Math.round(d.utilization_rate*100)+'%</div><div class=\"wm-stat-lbl\">利用率</div></div><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+d.items_worn+'/'+d.items_total+'</div><div class=\"wm-stat-lbl\">已穿/总数</div></div><div class=\"wm-stat-item\"><div class=\"wm-stat-val\">'+d.rated_count+'</div><div class=\"wm-stat-lbl\">有评分</div></div></div></div>';if(d.top_styles&&d.top_styles.length){{html+='<div class=\"wm-card\"><div class=\"wm-title\">🎯 最爱风格</div>';var maxS=d.top_styles[0].count;d.top_styles.forEach(function(s){{var pct=Math.round(s.count/maxS*100);html+='<div class=\"wm-bar-row\"><span class=\"wm-bar-label\">'+escHtml(s.name)+'</span><div class=\"wm-bar-track\"><div class=\"wm-bar-fill\" style=\"width:'+pct+'%\"></div></div><span class=\"wm-bar-num\">'+s.count+'</span></div>'}});html+='</div>'}}if(d.top_items&&d.top_items.length){{html+='<div class=\"wm-card\"><div class=\"wm-title\">👟 最爱单品</div>';var maxI=d.top_items[0].count;d.top_items.forEach(function(s){{var pct=Math.round(s.count/maxI*100);html+='<div class=\"wm-bar-row\"><span class=\"wm-bar-label\">'+escHtml(s.id)+'</span><div class=\"wm-bar-track\"><div class=\"wm-bar-fill\" style=\"width:'+pct+'%\"></div></div><span class=\"wm-bar-num\">'+s.count+'</span></div>'}});html+='</div>'}}html+='</div>';el.innerHTML=html}},false)}}
...
/* ═══ 探索页 ═══ */
function renderStyleCards(styles,showDesc){{if(!styles||!styles.length)return'<div class="wrd-empty">暂无风格数据</div>';return styles.map(function(s){{var desc=showDesc&&s.description?'<div class="es-desc">'+escHtml(s.description)+'</div>':'';var hasImg=!!s.image;var iconHtml=hasImg?'<img src="'+escHtml(s.image)+'" alt="'+escHtml(s.name_zh)+'" loading="lazy" onclick="event.stopPropagation();showImg(\\''+escHtml(s.image)+'\\')" style="cursor:pointer">':s.name_zh.charAt(0);var iconCls=hasImg?'es-icon has-img':'es-icon';var bottomHtml='';var items=[];if(s.top_items)items=s.top_items;if(items.length>0){{var chips=items.map(function(it){{var thumb=it.thumb?'<img src="'+escHtml(it.thumb)+'" loading="lazy" onclick="event.stopPropagation();showImg(\\''+escHtml(it.thumb)+'\\')">':'';return'<span class="es-item-chip">'+thumb+'</span>'}}).join('');bottomHtml='<div class="es-bottom"><div class="es-items">'+chips+'</div><button class="es-try-btn" onclick="event.stopPropagation();tryStyle(\\''+escHtml(s.id)+'\\',this)">🧪 试穿</button></div>'}}return'<div class="exp-style-card" onclick="window.location=\\'/style/'+escHtml(s.id)+'\\'"><div class="es-header"><div class="'+iconCls+'">'+iconHtml+'</div><div class="es-info"><div class="es-name">'+escHtml(s.name_zh)+'</div><div class="es-en">'+escHtml(s.name_en||s.id)+'</div></div></div>'+desc+bottomHtml+'</div>'}}).join('')}}
function loadExploreTweak(){{var el=document.getElementById('exp-tweak-content');var _loaded=__apiCache['exp-tweak']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-tweak','/api/explore/tweak',function(d,fromCache){{el.innerHTML=d?renderStyleCards(d.styles,true)||'<div class="wrd-empty">先完成几套穿搭推荐吧</div>':'<div class="wrd-empty">加载失败</div>'}})}}
function loadExploreTransform(){{var el=document.getElementById('exp-transform-content');var _loaded=__apiCache['exp-transform']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-transform','/api/explore/transform',function(d,fromCache){{el.innerHTML=d?renderStyleCards(d.styles,true)||'<div class="wrd-empty">已探索全部风格</div>':'<div class="wrd-empty">加载失败</div>'}})}}
function loadExploreCross(){{var el=document.getElementById('exp-cross-content');var _loaded=__apiCache['exp-cross']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-cross','/api/explore/cross',function(d,fromCache){{if(!d){{el.innerHTML='<div class="wrd-empty">加载失败</div>';return}}var html='';if(d.fusion)html+='<div class="es-fusion">🎲 '+escHtml(d.fusion)+'</div>';html+=renderStyleCards(d.styles,true);el.innerHTML=html||'<div class="wrd-empty">暂无跨界建议</div>'}})}}
function loadExploreTrends(){{var el=document.getElementById('exp-trends-content');var _loaded=__apiCache['exp-trends']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-trends','/api/explore/trends',function(d,fromCache){{el.innerHTML=d?'<div class="section-header">📚 全部风格 ('+d.total+')</div>'+renderStyleCards(d.styles,false):'<div class="wrd-empty">加载失败</div>'}})}}
...
function clearAddImages(){{__addImages=[];__addAnalysisData=null;__matchResults=null;__selectedMatchIds=[];__previewOutfitData=null;document.getElementById('add-image-strip').style.display='none';document.getElementById('add-image-strip').innerHTML='';document.getElementById('add-result').style.display='none';document.getElementById('add-result').innerHTML='';document.getElementById('add-progress').style.display='none';document.getElementById('add-action-cards').style.display='grid';var btn=document.getElementById('add-confirm-btn');btn.disabled=true;btn.textContent='确认分析';btn.onclick=function(){{submitAddImages()}};document.getElementById('add-camera-input').value='';document.getElementById('add-album-input').value=''}}
