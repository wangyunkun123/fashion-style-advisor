...
def _get_user_rating_profile():
    """
    分析用户近期评分行为，返回探索倾向画像。
    Returns:
      total_ratings: 总评分数
      recent_3star_streak: 最近连续3星次数
      recent_1star_streak: 最近连续1星次数
      exploration_appetite: -1(保守) ~ 0(中性) ~ +1(冒险)
      unexplored_ratio: 未探索风格占比
    """
    ratings = load_all_ratings()
    if not ratings:
        return {
            'total_ratings': 0,
            'recent_3star_streak': 0,
            'recent_1star_streak': 0,
            'exploration_appetite': 0.0,
            'unexplored_ratio': 1.0,
            'is_new_user': True,
        }

    # 按时间排序
    ratings_sorted = sorted(ratings, key=lambda r: r.get('timestamp', ''), reverse=True)
    ...
    # 探索倾向：3星比例高→更愿意探索，1星比例高→更保守
    three_star_ratio = sum(1 for r in ratings if r.get('rating', 0) >= 3) / max(total, 1)
    one_star_ratio = sum(1 for r in ratings if r.get('rating', 0) <= 1) / max(total, 1)
    appetite = (three_star_ratio - one_star_ratio) * 0.8  # 范围 -0.8 ~ +0.8

    # 未探索比例
    zone = get_user_comfort_zone()
    total_styles = len(load_all_styles())
    unexplored = len(zone.get('unexplored_styles', []))
    unexplored_ratio = unexplored / max(total_styles, 1)

    return {
        'total_ratings': total,
        'recent_3star_streak': streak_3,
        'recent_1star_streak': streak_1,
        'exploration_appetite': round(appetite, 2),
        'unexplored_ratio': round(unexplored_ratio, 2),
        'is_new_user': total < 5,
    }
...
def should_use_bline(state=None):
    """
    自适应 B线触发：根据用户探索倾向动态调整频率。

    规则:
      - 新用户（<5条评分）→ 每5次推1次B线（保守起步）
      - 连续3次3星 → 每2-3次推1次（提高探索频率）
      - 连续2次1星 → 暂停B线3轮（退回安全区）
      - 未探索率 > 50% → 提高频率（有很多可探索的）
      - 默认: 每4次推1次
    """
    if state is None:
        state = load_state()
    ...
    if total == 0:
        return False  # 第一次用 A线

    profile = _get_user_rating_profile()

    # 安全阀：连续1星 → 暂停探索
    if profile['recent_1star_streak'] >= 2:
        last_bline = state.get('last_bline_at', 0)
        cooldown = total - last_bline if last_bline else 999
        if cooldown < 3:  # 3轮内不再触发
            return False
    ...
    if profile['is_new_user']:
        interval = 5  # 新用户保守
    elif profile['recent_3star_streak'] >= 3:
        interval = 2  # 用户很满意，加速探索
    elif profile['exploration_appetite'] > 0.3:
        interval = 3  # 倾向探索
    elif profile['unexplored_ratio'] > 0.5 and total > 10:
        interval = 3  # 还有很多没试过的风格
    ...
    last_bline = state.get('last_bline_at', 0)
    if last_bline == 0:
        return total >= interval  # 首次 B线至少等 interval 轮
    return (total - last_bline) >= interval


def should_use_bold(state=None):
    """
    自适应大胆触发：根据用户探索深度动态调整。

    规则:
      - 新用户 → 不触发大胆
      - 连续3次3星 + 微调也3星 → 解锁大胆（用户接受度高）
      - 连续1星 → 不触发
      - 未探索率 < 20% → 触发（几乎都试过了，需要更大胆）
      - 默认: B线内每3-5次触发1次（根据 appetite）
    """
    if state is None:
        state = load_state()
    ...
    if profile['recent_3star_streak'] >= 4:
        interval = 2  # 用户持续满意，加速大胆
    elif profile['exploration_appetite'] > 0.5:
        interval = 3  # 高探索欲
    elif profile['unexplored_ratio'] < 0.2:
        interval = 3  # 已探索大部分，需要更冒险
    elif profile['exploration_appetite'] < -0.3:
        interval = 6  # 用户偏保守，降低频率
    ...
    # 检查间隔
    last_bold = state.get('last_bold_at', 0)
    if last_bold == 0:
        return bline >= interval
    return (bline - last_bold) >= interval


def increment_state(state, is_bline=False, is_bold=False, item_ids=None):
    """递增计数并记录穿着"""
    total = state.get('total_recommendations', 0) + 1
    state['total_recommendations'] = total
    if is_bline:
        state['bline_count'] = state.get('bline_count', 0) + 1
        state['last_bline_at'] = total
    if is_bold:
        state['bold_count'] = state.get('bold_count', 0) + 1
        state['last_bold_at'] = state.get('bline_count', 0)
    if item_ids:
...
def generate_exploration_directions(anchor_item, weather_temp=None,
    ...
    """
    为锚点单品生成 2-3 个探索方向，三级渐进式跨界：

    boldness='safe':   adjacent only — 日常穿搭（仅舒适区）
    boldness='micro':  adjacent + step — 改变自己（舒适区 + 关联风格）
    boldness='bold':   step + leap — 大胆跨界（从舒适区边缘逐步跨越，不再一刀切排除）

    每级有不同风险控制：
      - adjacent: 无额外限制，匹配分 ≥ 40
      - step: 1 个探索策略，匹配分 ≥ 30
      - leap: 2 个探索策略叠加，匹配分 ≥ 15，需提供安全替代
    """
    if comfort_zone is None:
        comfort_zone = get_user_comfort_zone()
    ...
    if boldness == 'safe':
        # ── 日常穿搭：仅舒适区 ──
        candidate_styles = set(comfort_styles)
        min_score = 40  # 高门槛，确保品质

    elif boldness == 'micro':
        # ── 改变自己：舒适区 + related_styles + 流行趋势 ──
        candidate_styles = set(comfort_styles)
        ...
        # ── 大胆跨界：step + leap，不再完全排除舒适区 ──
        # 保留舒适区 related_styles 作为 step 候选
        ...
        # 候选源 1：未探索风格（leap）
        candidate_styles |= set(unexplored) - set(comfort_styles)

        # 候选源 2：舒适区的 conflicting_styles（leap）
        for cs in comfort_styles:
            style = all_styles.get(cs, {})
            for cfs in style.get('conflicting_styles', []):
                if cfs not in disliked:
                    candidate_styles.add(cfs)

        # 候选源 3：锚点低分风格（15-35 分 — 真正的跨界挑战，leap）
        item_scores_all = score_cache.get(anchor_item['clothing_id'], {})
        low_score_styles = [sid for sid, sdata in item_scores_all.items()
                           if 15 <= sdata.get('score', 0) <= 35
                           and sid not in disliked]
        candidate_styles.update(low_score_styles)
    ...
    for sid in candidate_styles:
        ...
        if distance == 'adjacent':
            rationale = f"安心之选——「{appeal['visual_signature']}」在{style['name_zh']}中的经典演绎，你的舒适区基础款"
        elif distance == 'step':
            rationale = f"渐进探索——从你喜欢的风格延伸而来，「{appeal['visual_signature']}」作为桥梁单品带你走进{style['name_zh']}"
        else:
            rationale = f"大胆跨界！「{appeal['visual_signature']}」的{appeal['fabric_story']}与{style['name_zh']}碰撞，可能诞生你的专属风格"

        # 风险控制：leap 距离需提供安全替代标记
        risk_note = ''
        if distance == 'leap':
            risk_note = '⚠️ 高风险高回报 — 建议同时查看「今天也适合」的安全替代'

        # 综合分：anchor 匹配分 + 新颖度加成 + 距离惩罚
        novelty_bonus = 0
        if distance == 'leap':
            novelty_bonus = 15
        elif distance == 'step':
    ...
    # 优先取每种距离的最高分，保证多样性
    result = []
    for dist in ['adjacent', 'step', 'leap']:
        dist_candidates = [c for c in candidates if c['comfort_distance'] == dist]
        dist_candidates.sort(key=lambda x: -x['composite'])
        result.extend(dist_candidates[:1])  # 每种距离取 1 个

    # 如果不足 3 个，用剩余最高分补足
    remaining = [c for c in candidates if c not in result]
    remaining.sort(key=lambda x: -x['composite'])
...
def prepare_bline_outfit(anchor_item, companions, direction, weather_temp=30, weather_cond='晴',
    ...
    bold_label = '大胆跨界' if boldness == 'leap' else '微调探索'

    style_notes_lines = [
        f'- {style_name}：{style_desc[:12]}',
        f'- {anchor_note}',
        f'- 穿法：{wear_tip}',
        f'- [{bold_label}] 新方向尝试',
    ]
