...
function checkDailyStatus(){{fetch('/health').then(function(r){{return r.json()}}).then(function(d){{var warn=document.getElementById('daily-warning');var load=document.getElementById('daily-loading');var hero=document.querySelector('.hero-card');if(!warn)return;if(d.today_ok){{warn.style.display='none';load.style.display='none';if(hero)hero.style.opacity='1';return}}if(d.running){{warn.style.display='none';load.style.display='block';if(hero)hero.style.opacity='0.4';document.getElementById('daily-loading-text').textContent='正在生成今日穿搭...';setTimeout(checkDailyStatus,5000);return}}warn.style.display='block';load.style.display='none';if(hero)hero.style.opacity='0.4'}}).catch(function(e){{console.log('Health check failed:',e)}})}}
