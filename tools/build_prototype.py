...
function filterTodayCards(){{var t=new Date();var ds=t.getFullYear()+'-'+('0'+(t.getMonth()+1)).slice(-2)+'-'+('0'+t.getDate()).slice(-2);var list=document.getElementById('today-list');if(!list)return;var cards=list.querySelectorAll('.fav-card');var n=0;for(var i=0;i<cards.length;i++){{var d=cards[i].getAttribute('data-date');if(d!==ds)cards[i].style.display='none';else{{cards[i].style.display='';n++}}}}if(!n&&cards.length){{var p=list.querySelector('.today-empty')||document.createElement('div');p.className='today-empty';p.style.cssText='padding:16px;color:var(--muted);font-size:13px';p.textContent='今日暂无推荐';if(!p.parentNode)list.appendChild(p)}}}}}}
document.addEventListener('DOMContentLoaded',filterTodayCards);
