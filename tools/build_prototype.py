...
function removeChip(groupId,idx){{var data=__currentItemData;if(!data)return;if(groupId==='brand'){{if(data.brand)data.brand.name=''}}else if(groupId==='color'){{var cTags=getTagGroups(data).find(function(g){{return g.id==='color'}});if(cTags&&idx<cTags.tags.length){{var oldVal=cTags.tags[idx];var c=data.color||{{}};if(oldVal===c.hue_family)c.hue_family='';else if(oldVal===c.hue_name)c.hue_name='';else if(oldVal===c.saturation)c.saturation='';else if(oldVal===c.lightness)c.lightness='';else if(oldVal==='中性色')c.is_neutral=false;else if(oldVal==='显白')c.friendly_for_pale_skin=false}}}}else if(groupId==='fabric'){{var fTags=getTagGroups(data).find(function(g){{return g.id==='fabric'}});if(fTags&&idx<fTags.tags.length){{var oldVal=fTags.tags[idx];var f=data.fabric||{{}};if(oldVal===f.primary)f.primary='';else if(oldVal===f.texture)f.texture='';else if(oldVal===f.weight)f.weight=''}}}}else if(groupId==='silhouette'){{var sTags=getTagGroups(data).find(function(g){{return g.id==='silhouette'}});if(sTags&&idx<sTags.tags.length){{var oldVal=sTags.tags[idx];var s=data.silhouette||{{}};if(oldVal===s.fit)s.fit='';else if(oldVal===s.shoulder_effect)s.shoulder_effect='';else if(oldVal===s.torso_effect)s.torso_effect=''}}}}else if(groupId==='pattern'){{var pTags=getTagGroups(data).find(function(g){{return g.id==='pattern'}});if(pTags&&idx<pTags.tags.length){{var oldVal=pTags.tags[idx];var p=data.pattern||{{}};if(oldVal===p.type)p.type='';else if(oldVal===p.density)p.density='';if(oldVal==='Logo')p.logo_visible=false}}}}else if(groupId==='season'){{if(data.fabric&&data.fabric.seasonality)data.fabric.seasonality.splice(idx,1)}}else if(groupId==='occasions'){{if(data.occasions)data.occasions.splice(idx,1)}}else if(groupId==='style_modifiers'){{if(data.style_modifiers)data.style_modifiers.splice(idx,1)}}renderItemCard(data)}}
