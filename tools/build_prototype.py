...
function renderItemCardH(it){{var inner='';if(it.thumb){{inner='<img class=\"wrd-item-card-img\" src=\"'+__CDN__+escHtml(it.thumb)+'\" loading=\"lazy\" onerror=\"this.style.display=\\'none\\';this.parentElement.innerHTML=\\'<span style=font-size:11px;color:var(--muted)>'+escHtml(it.id)+'</span>\\'\">'}}else{{inner='<span style=\"font-size:11px;color:var(--muted)\">'+escHtml(it.id)+'</span>'}}var badgeHtml='';if(__newItemIds&&__newItemIds.indexOf(it.id)!==-1){{badgeHtml='<span class=\"new-badge\">NEW</span>'}}return'<div class=\"wrd-item-card-h\" onclick=\"openItemModal(\\''+escHtml(it.id)+'\\')\" style=\"position:relative\"><div class=\"wrd-item-card-img-wrap\">'+inner+'<span class=\"wrd-item-card-id\">'+escHtml(it.id)+'</span></div>'+badgeHtml+'</div>'}}
