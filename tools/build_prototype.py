...
def extract_palette(outfit):
    """从服装原始色彩中提取配色（读 outfit.md 表格颜色列 → hex）"""
                        ...
                        if os.path.exists(tag_path):
                            try:
                                with open(tag_path) as tf:
                                    tag = json.load(tf)
                                tag_color = tag.get('color', {}).get('hue_name', '')
                                if tag_color:
                                    color_text = tag_color
                            except Exception:
                                pass
...
.im-cat-picker{{display:flex;flex-wrap:wrap;gap:6px;padding:4px 0}}
.im-cat-opt{{display:flex;align-items:center;gap:4px;padding:8px 10px;border-radius:20px;font-size:12px;font-weight:500;color:var(--sub);background:#f5f6f8;cursor:pointer;transition:all .15s;-webkit-tap-highlight-color:transparent;border:1.5px solid transparent}}
.im-cat-opt:active{{transform:scale(.95)}}
.im-cat-opt-sel{{background:var(--navy);color:#fff;border-color:var(--navy);font-weight:600}}
.im-cat-opt span{{white-space:nowrap}}.cat-picker-overlay{{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.45);z-index:400;display:flex;align-items:flex-end;justify-content:center;animation:fadeIn .2s ease}}.cat-picker-sheet{{background:var(--white);border-radius:20px 20px 0 0;width:100%;max-height:70vh;overflow-y:auto;padding:16px 12px 24px;box-shadow:0 -4px 24px rgba(0,0,0,.12)}}.cat-picker-title{{font-size:15px;font-weight:700;color:var(--text);text-align:center;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border)}}@keyframes fadeIn{{from{{opacity:0}}to{{opacity:1}}}}
...
.profile-privacy{{font-size:11px;color:var(--muted);background:#f8fafb;border-radius:10px;padding:10px 14px;margin:12px 0 0;line-height:1.5;display:flex;align-items:flex-start;gap:6px}}
.profile-privacy svg{{width:14px;height:14px;color:var(--navy);flex-shrink:0;margin-top:1px}}
/* ═══ 折叠卡片 ═══ */
.collapse-card{{margin:14px 14px 0;background:var(--white);border-radius:var(--radius);overflow:hidden;box-shadow:var(--shadow);border:1px solid rgba(30,58,95,.05)}}
.collapse-header{{display:flex;align-items:center;justify-content:space-between;padding:16px 18px;cursor:pointer;user-select:none;-webkit-tap-highlight-color:transparent;transition:background .2s}}
.collapse-header:active{{background:#f8fafc}}
.collapse-header-left{{display:flex;align-items:center;gap:10px}}
.collapse-header-icon{{font-size:20px}}
.collapse-header-title{{font-size:15px;font-weight:700;color:var(--text)}}
.collapse-header-arrow{{font-size:12px;color:var(--muted);transition:transform .25s}}
.collapse-card.open .collapse-header-arrow{{transform:rotate(180deg)}}
.collapse-body{{display:none;padding:0 18px 18px}}
.collapse-card.open .collapse-body{{display:block}}
.future-section{{padding:20px 14px;text-align:center}}
.future-section-title{{font-size:12px;color:var(--muted);letter-spacing:1px;font-weight:600;margin-bottom:8px}}
.future-section-hint{{font-size:11px;color:#bcc4cd;line-height:1.6}}
/* 照片上传区 */
...
/* 风格选择器 */
.style-pick-area{{display:flex;flex-wrap:wrap;gap:6px;align-items:center;min-height:34px;padding:6px 0}}
.style-chip{{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;background:#eef2f7;border-radius:14px;font-size:12px;font-weight:600;color:var(--navy)}}
.style-chip .sc-remove{{font-size:13px;cursor:pointer;opacity:.5;line-height:1;-webkit-tap-highlight-color:transparent}}
.style-chip .sc-remove:active{{opacity:1}}
.style-pick-btn{{padding:8px 16px;background:#fff;border:2px dashed #dce3ed;border-radius:12px;font-size:12px;color:var(--muted);cursor:pointer;-webkit-tap-highlight-color:transparent;transition:all .2s}}
.style-pick-btn:active{{background:#f0f4fa;border-color:var(--navy)}}
.style-pick-clear{{font-size:11px;color:var(--muted);cursor:pointer;padding:4px 8px;-webkit-tap-highlight-color:transparent}}
/* 风格选择弹窗 */
.style-picker-modal{{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(26,40,56,.7);z-index:200;flex-direction:column;-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px)}}
.style-picker-modal.show{{display:flex}}
.style-picker-sheet{{background:var(--white);border-radius:var(--radius) var(--radius) 0 0;width:100%;max-width:500px;max-height:88vh;display:flex;flex-direction:column;animation:slideUp .3s ease;margin-top:auto;margin-left:auto;margin-right:auto}}
.style-picker-header{{display:flex;align-items:center;justify-content:space-between;padding:16px 18px;border-bottom:1px solid var(--border)}}
.style-picker-title{{font-size:16px;font-weight:700;color:var(--text)}}
.style-picker-close{{width:32px;height:32px;border-radius:50%;border:none;background:#f0f4f8;color:var(--sub);font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;-webkit-tap-highlight-color:transparent}}
.style-picker-scroll{{flex:1;overflow-y:auto;padding:12px 18px}}
.style-picker-grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}}
.style-picker-card{{padding:14px 12px;border:2px solid #eef2f7;border-radius:12px;cursor:pointer;transition:all .2s;text-align:center;-webkit-tap-highlight-color:transparent}}
.style-picker-card:active{{transform:scale(.97)}}
.style-picker-card.selected{{border-color:var(--navy);background:#f0f4fa}}
.style-picker-card .spc-name{{font-size:13px;font-weight:700;color:var(--text);margin-bottom:2px}}
.style-picker-card .spc-en{{font-size:10px;color:var(--muted)}}
.style-picker-card .spc-tc{{font-size:9px;display:inline-block;padding:1px 6px;border-radius:6px;margin-top:4px}}
.style-picker-footer{{padding:12px 18px;border-top:1px solid var(--border);display:flex;gap:8px}}
.style-picker-footer button{{flex:1;padding:12px;border:none;border-radius:12px;font-size:14px;font-weight:600;cursor:pointer;-webkit-tap-highlight-color:transparent}}
.spf-clear{{background:#f5f0eb;color:#c4523c}}
.spf-confirm{{background:var(--navy);color:#fff}}
...
<!-- ═══ 形象管理（折叠卡）═══ -->
<div class="collapse-card open" id="profile-card">
<div class="collapse-header" onclick="document.getElementById('profile-card').classList.toggle('open')">
<div class="collapse-header-left">
<span class="collapse-header-icon">👤</span>
<span class="collapse-header-title">形象管理</span>
</div>
<span class="collapse-header-arrow">▲</span>
</div>
...
<div class="style-pick-area" id="style-pick-chips"></div>
<div style="display:flex;gap:8px;align-items:center">
<button class="style-pick-btn" onclick="openStylePicker()">+ 从风格库选择</button>
<button class="style-pick-clear" onclick="clearStylePrefs()">清空</button>
</div>
</div>
...
<div class="future-section-title">更多功能即将上线</div>
<div class="future-section-hint">穿搭日历 · 风格分析 · 胶囊衣橱 · 更多…</div>
</div>
...
<!-- ═══ 风格选择弹窗 ═══ -->
<div class="style-picker-modal" id="style-picker-modal" onclick="if(event.target===this)closeStylePicker()">
<div class="style-picker-sheet">
<div class="style-picker-header">
<span class="style-picker-title">选择你喜欢的风格（最多5个）</span>
<button class="style-picker-close" onclick="closeStylePicker()">✕</button>
</div>
<div class="style-picker-scroll" id="style-picker-scroll">
<div class="style-picker-grid" id="style-picker-grid"></div>
</div>
<div class="style-picker-footer">
<button class="spf-clear" onclick="clearStylePrefs();closeStylePicker()">清空选择</button>
<button class="spf-confirm" onclick="confirmStylePicker()">确认</button>
</div>
</div>
</div>
...
var COLOR_MAP={{'黑色':'#2a2a2a','白色':'#f5f3ef','米白':'#f5f0e8','乳白':'#faf8f5','深灰':'#4a4a4a','灰色':'#9e9e9e','浅灰':'#d0d0d0','银灰':'#bdbdbd','灰绿':'#8a9a82','卡其':'#c4b5a0','卡其色':'#c4b5a0','驼色':'#b8976e','深棕':'#5c3d2e','棕色':'#7a5230','浅棕':'#b8956a','深蓝':'#1e3a5f','藏蓝':'#1e3a6f','藏青':'#1e3a5f','海军蓝':'#1e3a5f','蓝色':'#4a7eb5','浅蓝':'#7ea3c8','天蓝':'#8bb8d6','军绿':'#5c6e4a','军绿色':'#5c6e4a','墨绿':'#3c5032','绿色':'#6b8c5c','浅绿':'#9cba8c','正红色':'#c4523c','红色':'#c4523c','暗红':'#8b2e3e','酒红':'#8b2e3e','橙色':'#e88a3c','亮橙':'#f0983c','橘色':'#e88030','黄色':'#d4a84b','姜黄':'#c49a3c','米黄':'#e8d8b0','紫色':'#8b6b9e','浅紫':'#b89ac8','粉色':'#e8b4b8','浅粉':'#f0c8cc','米色':'#e8dcc8','沙色':'#d8ccb0','深牛仔蓝':'#2a4a6c','牛仔蓝':'#4a6a8c','浅牛仔蓝':'#7a9ab8','牛油果绿':'#7a9a5c','条纹':'#c0c0c0','印花':'#c0c0c0'}};function colorHex(name){{var c=COLOR_MAP[name];if(c)return c;for(var k in COLOR_MAP){{if(k.indexOf(name)!=-1||name.indexOf(k)!=-1)return COLOR_MAP[k]}}return'#bdbdbd'}}
	var CAT_ORDER=['TS','LS','SHIRT','BLOUSE','TANK','KNIT','JK','PT','SH','DRESS','SKIRT','JMP','SUIT','SHOE','BAG','HAT','SOCK','SUN','ACC'];
var CAT_ICONS={{'TS':'👕','LS':'👔','SHIRT':'👔','BLOUSE':'👚','TANK':'🎽','KNIT':'🧶','JK':'🧥','PT':'👖','SH':'🩳','DRESS':'👗','SKIRT':'💃','JMP':'🦺','SUIT':'👗','SHOE':'👟','BAG':'🎒','HAT':'🧢','SOCK':'🧦','SUN':'🕶️','ACC':'⌚'}};var CAT_CODE_TO_NAME={{'TS':'短袖上衣','LS':'长袖上衣','SHIRT':'衬衣','BLOUSE':'女士衬衫','TANK':'背心','KNIT':'针织衫','JK':'外套','PT':'长裤','SH':'短裤','DRESS':'连衣裙','SKIRT':'半身裙','JMP':'连体裤','SUIT':'套装','SHOE':'鞋子','BAG':'包','HAT':'帽子','SOCK':'袜子','SUN':'墨镜','ACC':'手部配饰'}};
...
function renderItemCardH(it){{var inner='';if(it.thumb){{var tp=(it.thumb||'').split('?')[0];inner='<img class=\"wrd-item-card-img\" src=\"/api/image?f='+encodeURIComponent(tp)+'&w=200\" loading=\"lazy\" onerror=\"this.style.display=\\'none\\';this.parentElement.innerHTML=\\'<span style=font-size:11px;color:var(--muted)>'+escHtml(it.id)+'</span>\\'\">'}}else{{inner='<span style=\"font-size:11px;color:var(--muted)\">'+escHtml(it.id)+'</span>'}}var badgeHtml='';if(__newItemIds&&__newItemIds.indexOf(it.id)!==-1){{badgeHtml='<span class=\"new-badge\">NEW</span>'}}return'<div class=\"wrd-item-card-h\" onclick=\"openItemModal(\\''+escHtml(it.id)+'\\')\" style=\"position:relative\"><div class=\"wrd-item-card-img-wrap\">'+inner+'<span class=\"wrd-item-card-id\">'+escHtml(it.id)+'</span></div>'+badgeHtml+'</div>'}}
...
function cancelChipEdit(){{__editingTagIdx=-1;__editingTagGroup='';__editingTagField='';document.getElementById('im-chip-editor').style.display='none'}}function showCatPicker(data){{var overlay=document.createElement('div');overlay.className='cat-picker-overlay';overlay.onclick=function(e){{if(e.target===overlay){{overlay.remove()}}}};var html='<div class="cat-picker-sheet"><div class="cat-picker-title">选择品类</div><div class="im-cat-picker">'+CAT_ORDER.map(function(code){{var icon=CAT_ICONS[code]||'📦';var name=CAT_CODE_TO_NAME[code]||code;var sel=code===data.category_code?' im-cat-opt-sel':'';return'<div class="im-cat-opt'+sel+'" onclick="(function(c){{var d=__currentItemData;if(!d||!CAT_CODE_TO_NAME[c])return;d.category_code=c;d.category=CAT_CODE_TO_NAME[c];document.querySelector(\\'.cat-picker-overlay\\').remove();renderItemCard(d)}})(\\''+code+'\\')">'+icon+'<span>'+name+'</span></div>'}}).join('')+'</div></div>';overlay.innerHTML=html;document.body.appendChild(overlay)}}
...
/* ═══ 探索页 ═══ */
function renderStyleCards(styles,showDesc,hintText){{if(!styles||!styles.length)return'<div class="wrd-empty">'+(hintText||'暂无风格数据')+'</div>';return styles.map(function(s){{var desc=showDesc&&s.description?'<div class="es-desc">'+escHtml(s.description)+'</div>':'';var hasImg=!!s.image;var fullImg=s.image_full||s.image||'';var iconHtml=hasImg?'<img data-src="'+escHtml(s.image)+'" alt="'+escHtml(s.name_zh)+'" onclick="event.stopPropagation();showImg(\\''+escHtml(fullImg)+'\\')" style="cursor:pointer;background:#eef2f7;min-height:48px">':s.name_zh.charAt(0);var iconCls=hasImg?'es-icon has-img lazy-img':'es-icon';var bottomHtml='';var items=[];if(s.top_items)items=s.top_items;if(items.length>0){{var chips=items.map(function(it){{var cutout=it.cutout||it.thumb||'';var thumb=it.thumb||'';var thumbHtml=thumb?'<img data-src="'+escHtml(thumb)+'" onclick="event.stopPropagation();showImg(\\''+escHtml(cutout)+'\\')" style="background:#eef2f7">':'';return'<span class="es-item-chip lazy-img">'+thumbHtml+'</span>'}}).join('');bottomHtml='<div class="es-bottom"><div class="es-items">'+chips+'</div><button class="es-try-btn" onclick="event.stopPropagation();tryStyle(\\''+escHtml(s.id)+'\\',this)">🧪 试穿</button></div>'}}return'<div class="exp-style-card" onclick="window.location=\\'/style/'+escHtml(s.id)+'\\'"><div class="es-header"><div class="'+iconCls+'">'+iconHtml+'</div><div class="es-info"><div class="es-name">'+escHtml(s.name_zh)+'</div><div class="es-en">'+escHtml(s.name_en||s.id)+'</div></div></div>'+desc+bottomHtml+'</div>'}}).join('')}}
function loadExploreTweak(){{var el=document.getElementById('exp-tweak-content');var _loaded=__apiCache['exp-tweak']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-tweak','/api/explore/tweak',function(d,fromCache){{el.innerHTML=d?renderStyleCards(d.styles,true,d.hint)||'<div class="wrd-empty">先完成几套穿搭推荐吧</div>':'<div class="wrd-empty">加载失败</div>';observeLazyImages(el)}})}}
function loadExploreTransform(){{var el=document.getElementById('exp-transform-content');var _loaded=__apiCache['exp-transform']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-transform','/api/explore/transform',function(d,fromCache){{el.innerHTML=d?renderStyleCards(d.styles,true,d.hint)||'<div class="wrd-empty">已探索全部风格</div>':'<div class="wrd-empty">加载失败</div>';observeLazyImages(el)}})}}
function loadExploreCross(){{var el=document.getElementById('exp-cross-content');var _loaded=__apiCache['exp-cross']!==undefined;if(!_loaded)el.innerHTML='<div class="wrd-loading">加载中...</div>';cachedFetch('exp-cross','/api/explore/cross',function(d,fromCache){{if(!d){{el.innerHTML='<div class="wrd-empty">加载失败</div>';return}}var html='';if(d.fusion)html+='<div class="es-fusion">🎲 '+escHtml(d.fusion)+'</div>';html+=renderStyleCards(d.styles,true,d.hint);el.innerHTML=html||'<div class="wrd-empty">暂无跨界建议</div>';observeLazyImages(el)}})}}
...
function switchBodyShapes(g){{var maleEl=document.getElementById('seg-body-male');var femaleEl=document.getElementById('seg-body-female');if(g==='female'||g==='女'){{if(maleEl)maleEl.style.display='none';if(femaleEl)femaleEl.style.display=''}}else{{if(maleEl)maleEl.style.display='';if(femaleEl)femaleEl.style.display='none'}}__genderVal=g}}
...
function updateProfileGuide(){{var emptyFields=[];var occ=document.getElementById('pf-occupation').value.trim();var pref=__stylePrefIds.length>0;var pain=document.getElementById('pf-pain-points').value.trim();if(!occ)emptyFields.push('职业');if(!pref)emptyFields.push('偏好');if(!pain)emptyFields.push('困扰');var guide=document.getElementById('profile-guide');if(emptyFields.length===3){{guide.style.display='block';guide.innerHTML='<span>✨ 职业、偏好、困扰均留空 — AI 将完全自由探索，可能发现你意想不到的风格</span>'}}else if(emptyFields.length===2){{guide.style.display='block';guide.innerHTML='<span>✨ '+emptyFields.join('、')+'留空 — AI 将在这两个方面自由探索</span>'}}else if(emptyFields.length===1){{guide.style.display='block';guide.innerHTML='<span>✨ '+emptyFields[0]+'留空 — AI 将不受限制自由发挥</span>'}}else{{guide.style.display='none'}};updateProfilePreview()}}
function updateProfilePreview(){{var p=document.getElementById('profile-preview');var t=document.getElementById('profile-preview-text');var gender=__genderVal||'男';var h=document.getElementById('pf-height').value.trim();var w=document.getElementById('pf-weight').value.trim();var age=document.getElementById('pf-age').value.trim();var bt=__bodyVal;var st=__skinVal;var should=__shoulderVal;var face=__faceVal;var occ=document.getElementById('pf-occupation').value.trim();var pref=getStylePrefsString();var pain=document.getElementById('pf-pain-points').value.trim();var secrets=document.getElementById('pf-body-secrets').value.trim();var parts=[];if(age)parts.push(age+'岁');parts.push(gender==='女'?'女性':'男性');if(h)parts.push(h+'cm');if(w)parts.push(w+'kg');if(bt)parts.push(bt);if(st)parts.push(st+'肤色');if(should)parts.push(should);if(face)parts.push(face);var extra=[];if(occ)extra.push('职业: '+occ);if(pref)extra.push('偏好: '+pref);if(pain)extra.push('困扰: '+pain);if(parts.length>3||extra.length||secrets){{p.style.display='block';var previewText='\"'+parts.join('，')+'\"';if(extra.length)previewText+='\\n'+extra.join(' · ');if(secrets)previewText+='\\n🔒 身材秘密: '+secrets;t.textContent=previewText;t.style.whiteSpace='pre-line'}}else{{p.style.display='none'}}}}
function analyzeProfilePhotos(){{var b64s=[];Object.keys(__profilePhotos).forEach(function(s){{b64s.push({{slot:s,b64:__profilePhotos[s]}})}});if(!b64s.length){{alert('请先上传至少一张照片');return}}var btn=document.getElementById('analyze-btn');btn.disabled=true;btn.textContent='📤 上传照片...';fetch('/api/profile/analyze',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{images:b64s}})}}).then(r=>r.json()).then(function(d){{btn.disabled=false;btn.textContent='🔍 AI 分析照片中的身形';if(d.ok&&d.analysis){{var a=d.analysis;if(a.gender){{__genderVal=a.gender;switchBodyShapes(a.gender)}}if(a.estimated_height_cm&&parseInt(a.estimated_height_cm)>0)document.getElementById('pf-height').value=a.estimated_height_cm;if(a.body_type){{__bodyVal=a.body_type;setSegVal('seg-body',a.body_type)}}if(a.skin_tone){{__skinVal=a.skin_tone;setSegVal('seg-skin',a.skin_tone)}}if(a.shoulder_type){{__shoulderVal=a.shoulder_type;setSegVal('seg-shoulder',a.shoulder_type)}}if(a.face_shape){{__faceVal=a.face_shape;setSegVal('seg-face',a.face_shape)}}updateProfilePreview();updateProfileGuide();alert('✅ AI 分析完成，请核对并修正')}}else{{alert('AI 分析失败，请手动填写')}}}}).catch(function(e){{btn.disabled=false;btn.textContent='🔍 AI 分析照片中的身形';alert('网络错误: '+e.message)}})}}
function setSegVal(groupId,val){{var grp=document.getElementById(groupId);if(!grp){{/* seg-body 兼容: 同时查找 male/female 容器 */if(groupId==='seg-body'){{grp=document.getElementById('seg-body-male')||document.getElementById('seg-body-female')}}if(!grp)return}}var found=false;grp.querySelectorAll('.seg-choice').forEach(function(c){{c.classList.remove('selected');if(c.dataset.val===val){{c.classList.add('selected');found=true}}}});if(!found&&groupId==='seg-body'){{/* 如果 male 容器里没找到，试 female 容器 */var altGrp=document.getElementById('seg-body-female');if(altGrp&&altGrp!==grp){{altGrp.querySelectorAll('.seg-choice').forEach(function(c){{c.classList.remove('selected');if(c.dataset.val===val){{c.classList.add('selected');found=true}}}})}}}}if(!found){{var first=grp.querySelector('.seg-choice');if(first)first.classList.add('selected')}}}}
function saveProfile(){{var btn=document.getElementById('profile-save-btn');btn.disabled=true;btn.textContent='保存中...';var h=document.getElementById('pf-height').value.trim();var w=document.getElementById('pf-weight').value.trim();var age=document.getElementById('pf-age').value.trim();var occ=document.getElementById('pf-occupation').value.trim();var pref=getStylePrefsString();var pain=document.getElementById('pf-pain-points').value.trim();var bodySecrets=document.getElementById('pf-body-secrets').value.trim();var useImg=document.getElementById('toggle-use-image').classList.contains('on');var data={{use_my_image:useImg,height_cm:h,weight_kg:w,age:age,body_type:__bodyVal,skin_tone:__skinVal,shoulder_type:__shoulderVal,face_shape:__faceVal,occupation:occ,style_preference:pref,pain_points:pain,body_secrets:bodySecrets}};fetch('/api/profile/save',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(data)}}).then(r=>r.json()).then(function(d){{if(d.ok){{btn.textContent='✅ 已保存';btn.classList.add('saved');setTimeout(function(){{btn.textContent='保存形象信息';btn.classList.remove('saved');btn.disabled=false}},2000)}}else{{btn.disabled=false;btn.textContent='保存失败，重试'}}}}).catch(function(e){{btn.disabled=false;btn.textContent='网络错误，重试'}})}}
...
/* ═══ 风格选择器 ═══ */
var __stylePrefIds=[];var __stylePrefNames=[];var __pickerStyles=[];var __tmpSelectedIds=[];
function renderPickerGrid(){{var grid=document.getElementById('style-picker-grid');if(!grid)return;var html='';__pickerStyles.forEach(function(s){{var sel=__tmpSelectedIds.indexOf(s.id)>=0;var tcLabel='';var tcColor='#888';if(s.trend_category==='popular_trend'){{tcLabel='🔥';tcColor='#E07B39'}}else if(s.trend_category==='classic'){{tcLabel='🏛️';tcColor='#2C3E50'}}else if(s.trend_category==='niche'){{tcLabel='🎭';tcColor='#7B4FBF'}}html+='<div class=\"style-picker-card'+(sel?' selected':'')+'\" onclick=\"togglePickerStyle(this,\\''+escHtml(s.id)+'\\')\"><div class=\"spc-name\">'+escHtml(s.name_zh)+'</div><div class=\"spc-en\">'+escHtml(s.name_en||s.id)+'</div><span class=\"spc-tc\" style=\"background:'+tcColor+'15;color:'+tcColor+'\">'+tcLabel+'</span></div>'}});grid.innerHTML=html||'<div style=\"padding:20px;text-align:center;color:var(--muted)\">暂无风格数据</div>'}}
function openStylePicker(){{__tmpSelectedIds=__stylePrefIds.slice();var grid=document.getElementById('style-picker-grid');if(!grid)return;grid.innerHTML='<div style=\"padding:20px;text-align:center;color:var(--muted)\">加载中...</div>';document.getElementById('style-picker-modal').classList.add('show');if(__pickerStyles.length){{renderPickerGrid()}}else{{fetch('/api/explore/trends').then(function(r){{return r.json()}}).then(function(d){{__pickerStyles=d.styles||[];renderPickerGrid()}}).catch(function(){{grid.innerHTML='<div style=\"padding:20px;text-align:center;color:var(--muted)\">加载失败</div>'}})}}}}
function closeStylePicker(){{document.getElementById('style-picker-modal').classList.remove('show')}}
function togglePickerStyle(card,styleId){{var idx=__tmpSelectedIds.indexOf(styleId);if(idx>=0){{__tmpSelectedIds.splice(idx,1);card.classList.remove('selected')}}else{{if(__tmpSelectedIds.length>=5){{alert('最多选择5个风格');return}}__tmpSelectedIds.push(styleId);card.classList.add('selected')}}}}
function confirmStylePicker(){{__stylePrefIds=__tmpSelectedIds.slice();var names=[];__stylePrefIds.forEach(function(sid){{var s=__pickerStyles.find(function(x){{return x.id===sid}});if(s)names.push(s.name_zh);else names.push(sid)}});__stylePrefNames=names;renderStyleChips();if(names.length){{document.getElementById('profile-preview');updateProfilePreview();updateProfileGuide()}}closeStylePicker()}}
function clearStylePrefs(){{__stylePrefIds=[];__stylePrefNames=[];__tmpSelectedIds=[];renderStyleChips();updateProfilePreview();updateProfileGuide()}}
function renderStyleChips(){{var area=document.getElementById('style-pick-chips');if(!area)return;if(!__stylePrefIds.length){{area.innerHTML='<span style=\"font-size:12px;color:var(--muted)\">未选择（AI将自由探索）</span>';return}}var html='';__stylePrefIds.forEach(function(sid,i){{html+='<span class=\"style-chip\">'+escHtml(__stylePrefNames[i]||sid)+'<span class=\"sc-remove\" onclick=\"event.stopPropagation();removeStylePref(\\''+escHtml(sid)+'\\')\">×</span></span>'}});area.innerHTML=html}}
function removeStylePref(sid){{var idx=__stylePrefIds.indexOf(sid);if(idx>=0){{__stylePrefIds.splice(idx,1);__stylePrefNames.splice(idx,1)}}renderStyleChips();updateProfilePreview();updateProfileGuide()}}
function setStylePrefsFromProfile(styleIdsStr){{if(!styleIdsStr){{clearStylePrefs();return}}var ids=styleIdsStr.split(',').map(function(s){{return s.trim()}}).filter(Boolean);__stylePrefIds=ids;__stylePrefNames=ids.slice();renderStyleChips()}}
function getStylePrefsString(){{return __stylePrefIds.join(', ')}}
/* ═══ 加载我的形象 ═══ */
var __profileLoaded=false;
function loadProfile(){{if(__profileLoaded)return;__profileLoaded=true;fetch('/api/profile').then(r=>r.json()).then(function(d){{var p=d.profile;if(!p)return;if(p.gender){{__genderVal=p.gender;switchBodyShapes(p.gender)}}if(p.height)document.getElementById('pf-height').value=p.height;if(p.weight)document.getElementById('pf-weight').value=p.weight;if(p.age)document.getElementById('pf-age').value=p.age;if(p.body_type){{__bodyVal=p.body_type;setSegVal('seg-body',p.body_type)}}if(p.skin_tone){{__skinVal=p.skin_tone;setSegVal('seg-skin',p.skin_tone)}}if(p.shoulder_type){{__shoulderVal=p.shoulder_type;setSegVal('seg-shoulder',p.shoulder_type)}}if(p.face_shape){{__faceVal=p.face_shape;setSegVal('seg-face',p.face_shape)}}if(p.occupation)document.getElementById('pf-occupation').value=p.occupation;if(p.style_preference||p.style_prefs){{var sp=p.style_preference||(p.style_prefs||[]).join(', ');setStylePrefsFromProfile(sp)}}else{{renderStyleChips()}}if(p.pain_points)document.getElementById('pf-pain-points').value=p.pain_points;if(p.body_secrets)document.getElementById('pf-body-secrets').value=p.body_secrets;var toggle=document.getElementById('toggle-use-image');var detail=document.getElementById('profile-detail');if(p.use_my_image===false){{toggle.classList.remove('on');detail.style.display='none'}}else{{toggle.classList.add('on');detail.style.display='block'}}/* load photos */if(p.photos){{Object.keys(p.photos).forEach(function(slot){{var slotId='slot-'+slot.replace(/_/g,'-').replace('full-body-front','full').replace('face-closeup','face').replace('full-body-side','side');var el=document.getElementById(slotId);if(el&&p.photos[slot]){{el.classList.add('has-photo');var img=el.querySelector('.photo-slot-preview');if(img)img.src=(__USER__?'/api/image?f=':'../')+p.photos[slot]+'?t='+Date.now()}}}})}}updateProfilePreview();updateProfileGuide()}}).catch(function(){{}})}}
function setPreference(mode){{fetch('/setpref?mode='+mode).catch(function(){{}})}}
/* ═══ 添加页 ═══ */
var __addImages=[];
function triggerAddCamera(){{var old=document.getElementById('add-camera-input');if(old)old.remove();var input=document.createElement('input');input.type='file';input.id='add-camera-input';input.accept='image/*';input.setAttribute('capture','environment');input.style.display='none';input.addEventListener('change',function(){{handleAddImages(this)}});document.body.appendChild(input);input.click()}}
function triggerAddAlbum(){{var old=document.getElementById('add-album-input');if(old)old.remove();var input=document.createElement('input');input.type='file';input.id='add-album-input';input.accept='image/*';input.multiple=true;input.style.display='none';input.addEventListener('change',function(){{handleAddImages(this)}});document.body.appendChild(input);input.click()}}
function handleAddImages(input){{var files=Array.from(input.files);if(!files.length)return;/* 客户端去重：按文件名+大小过滤5分钟内重复（iOS Safari 缓存兜底） */if(!window.__seenFileKeys)window.__seenFileKeys={{}};var now=Date.now();var unique=[];for(var i=0;i<files.length;i++){{var f=files[i];var key=f.name+'|'+f.size+'|'+f.lastModified;if(!window.__seenFileKeys[key]||now-window.__seenFileKeys[key]>300000){{window.__seenFileKeys[key]=now;unique.push(f)}}}}if(!unique.length)return;for(var k in window.__seenFileKeys){{if(now-window.__seenFileKeys[k]>300000)delete window.__seenFileKeys[k]}}/* 清除旧选图 */__addImages.forEach(function(img){{URL.revokeObjectURL(img.preview)}});__addImages=[];var total=unique.length;var loaded=0;unique.forEach(function(f){{var blobUrl=URL.createObjectURL(f);__addImages.push({{file:f,preview:blobUrl,compressed:null}});loaded++;if(loaded===total){{renderAddImageStrip()}}}});input.remove()}}
function renderAddImageStrip(){{var strip=document.getElementById('add-image-strip');var hasImgs=__addImages.length>0;strip.style.display=hasImgs?'flex':'none';var html='';__addImages.forEach(function(img,i){{var name=(img.file&&img.file.name)?img.file.name.replace(/\.[^.]+$/, ''):'图'+(i+1);if(name.length>8)name=name.slice(0,7)+'…';html+='<div class="add-image-thumb"><img src="'+img.preview+'"><span class="thumb-label">'+escHtml(name)+'</span><span class="thumb-remove" onclick="event.stopPropagation();removeAddImage('+i+')">&times;</span></div>'}});html+='<div class="add-image-thumb add-more-btn" onclick="triggerAddAlbum()"><span class="add-more-plus">+</span><span class="add-more-label">添加</span></div>';strip.innerHTML=html;document.getElementById('add-confirm-btn').disabled=!hasImgs;document.getElementById('add-action-cards').style.display=hasImgs?'none':'grid'}}
...
function clearAddImages(){{__addImages.forEach(function(img){{URL.revokeObjectURL(img.preview)}});__addImages=[];__addAnalysisData=null;__matchResults=null;__selectedMatchIds=[];__previewOutfitData=null;__addPollStopped=false;document.getElementById('add-image-strip').style.display='none';document.getElementById('add-image-strip').innerHTML='';document.getElementById('add-result').style.display='none';document.getElementById('add-result').innerHTML='';document.getElementById('add-progress').style.display='none';document.getElementById('add-action-cards').style.display='grid';var btn=document.getElementById('add-confirm-btn');btn.disabled=true;btn.textContent='确认分析';btn.onclick=function(){{submitAddImages()}};var ci=document.getElementById('add-camera-input');if(ci)ci.remove();var ai=document.getElementById('add-album-input');if(ai)ai.remove()}}
function compressImageV2(file,callback,errCallback){{try{{var blobUrl=URL.createObjectURL(file);var img=new Image();img.onload=function(){{URL.revokeObjectURL(blobUrl);var w=img.width,h=img.height;var maxW=720;if(w>maxW){{h=Math.round(h*maxW/w);w=maxW}}var c=document.createElement('canvas');c.width=w;c.height=h;var ctx=c.getContext('2d');ctx.drawImage(img,0,0,w,h);/* 质量阶梯：确保 <100KB（慢速网络友好）*/function tryQ(q){{c.toBlob(function(blob){{if(blob&&blob.size>100000&&q>0.2){{tryQ(q-0.15)}}else{{callback(blob||file)}}}},'image/jpeg',q)}}tryQ(0.45)}};img.onerror=function(){{URL.revokeObjectURL(blobUrl);if(errCallback)errCallback('图片解码失败');else callback(file)}};img.src=blobUrl}}catch(e){{if(errCallback)errCallback(e.message);else callback(file)}}}}
function submitAddImages(){{if(!__addImages.length)return;__addPollStopped=false;var btn=document.getElementById('add-confirm-btn');btn.disabled=true;btn.textContent='压缩中...';document.getElementById('add-progress').style.display='block';document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-loading\">压缩图片中...</div>';var compressedCount=0;var fails=0;var total=__addImages.length;var formData=new FormData();function doUpload(retryCount){{retryCount=retryCount||0;var estSize=0;__addImages.forEach(function(img){{if(img.compressed)estSize+=img.compressed.size}});if(estSize>450000){{document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-empty\" style=\"color:#e74c3c\">⚠️ 图片过大 ('+Math.round(estSize/1024)+'KB)<br><span style=\"font-size:11px;color:var(--muted)\">外网隧道限制 ≤500KB，请截图后重试</span><br><button onclick=\"submitAddImages()\" style=\"margin-top:10px;padding:10px 24px;background:var(--navy);color:#fff;border:none;border-radius:20px;font-size:14px\">🔄 重试上传</button></div>';btn.textContent='重试';btn.disabled=false;return}}document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-loading\">上传中 ('+Math.round(estSize/1024)+'KB)'+(retryCount>0?' · 第'+(retryCount+1)+'次':'')+'...</div>';var ctrl=new AbortController();var tid=setTimeout(function(){{ctrl.abort()}},60000);fetch('/api/wardrobe/add',{{method:'POST',body:formData,signal:ctrl.signal}}).then(function(r){{clearTimeout(tid);return r.json()}}).then(function(d){{if(d.task_id){{pollAddTask(d.task_id)}}else if(d.error){{var msg=d.message||d.error;document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-empty\" style=\"color:#e74c3c\">⚠️ '+escHtml(msg)+'<br><button onclick=\"submitAddImages()\" style=\"margin-top:10px;padding:10px 24px;background:var(--navy);color:#fff;border:none;border-radius:20px;font-size:14px\">🔄 重试上传</button></div>';btn.textContent='重试';btn.disabled=false}}else{{showAddReview(d)}}}}).catch(function(e){{clearTimeout(tid);if((e.name==='AbortError'||e.message==='Load failed')&&retryCount<1){{document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-loading\">网络超时，正在自动重试...</div>';setTimeout(function(){{doUpload(retryCount+1)}},1000)}}else{{var hint=retryCount>0?'重试后仍失败，请检查网络或切换WiFi':'可能是网络超时或图片过大';document.getElementById('add-progress-text').innerHTML='<div class=\\"wrd-empty\\" style=\\"color:#e74c3c\\">⚠️ 上传失败<br><span style=\\"font-size:11px;color:var(--muted)\\">'+hint+'</span><br><button onclick=\\"submitAddImages()\\" style=\\"margin-top:10px;padding:10px 24px;background:var(--navy);color:#fff;border:none;border-radius:20px;font-size:14px\\">🔄 重试上传</button></div>';btn.textContent='重试';btn.disabled=false}}}})}}__addImages.forEach(function(img,i){{if(img.compressed){{formData.append('images',img.compressed,img.file.name||'photo.jpg');compressedCount++;if(compressedCount+fails===total)doUpload();return}}compressImageV2(img.file,function(blob){{img.compressed=blob;formData.append('images',blob,img.file.name||'photo.jpg');compressedCount++;document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-loading\">压缩中 ('+compressedCount+'/'+total+')...</div>';if(compressedCount+fails===total)doUpload()}},function(err){{fails++;document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-loading\">压缩失败 ('+fails+'/'+total+')...</div>';if(fails===total){{document.getElementById('add-progress-text').innerHTML='<div class=\"wrd-empty\" style=\"color:#e74c3c\">❌ 所有图片压缩失败<br><span style=\"font-size:11px;color:var(--muted)\">该图片格式可能不兼容，请尝试截图后上传</span><br><button onclick=\"submitAddImages()\" style=\"margin-top:10px;padding:10px 24px;background:var(--navy);color:#fff;border:none;border-radius:20px;font-size:14px\">🔄 重试</button></div>';btn.textContent='重试';btn.disabled=false}}else if(compressedCount+fails===total){{doUpload()}}}})}})}}
...
/* ═══ 🆕 衣橱匹配 + AI 穿搭预览 ═══ */
var CAT_ICON_MAP={{'TS':'👕','LS':'👔','SHIRT':'👔','BLOUSE':'👚','TANK':'🎽','KNIT':'🧶','JK':'🧥','PT':'👖','SH':'🩳','DRESS':'👗','SKIRT':'💃','JMP':'🦺','SUIT':'👗','SHOE':'👟','BAG':'🎒','HAT':'🧢','SOCK':'🧦','SUN':'🕶️','ACC':'⌚'}};
