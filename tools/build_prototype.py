...
function loadStylePickerStyles(){{if(__pickerStyles.length)return;fetch('/api/explore/trends').then(function(r){{return r.json()}}).then(function(d){{__pickerStyles=d.styles||[]}}).catch(function(){{}})}};
function openStylePicker(){{loadStylePickerStyles();__tmpSelectedIds=__stylePrefIds.slice();var grid=document.getElementById('style-picker-grid');if(!grid)return;var html='';__pickerStyles.forEach(function(s){{var sel=__tmpSelectedIds.indexOf(s.id)>=0;var tcLabel='';var tcColor='#888';if(s.trend_category==='popular_trend'){{tcLabel='🔥';tcColor='#E07B39'}}else if(s.trend_category==='classic'){{tcLabel='🏛️';tcColor='#2C3E50'}}else if(s.trend_category==='niche'){{tcLabel='🎭';tcColor='#7B4FBF'}}html+='<div class=\"style-picker-card'+(sel?' selected':'')+'\" onclick=\"togglePickerStyle(this,\\''+escHtml(s.id)+'\\')\"><div class=\"spc-name\">'+escHtml(s.name_zh)+'</div><div class=\"spc-en\">'+escHtml(s.name_en||s.id)+'</div><span class=\"spc-tc\" style=\"background:'+tcColor+'15;color:'+tcColor+'\">'+tcLabel+'</span></div>'}});grid.innerHTML=html||'<div style=\"padding:20px;text-align:center;color:var(--muted)\">加载中...</div>';document.getElementById('style-picker-modal').classList.add('show')}}
