...
function handleAddImages(input){{var files=Array.from(input.files);if(!files.length)return;var loaded=0;var total=files.length;var statusEl=document.getElementById('add-progress-text');files.forEach(function(f){{compressImage(f,1024,0.75,function(b64){{__addImages.push({{file:f,preview:b64}});loaded++;if(statusEl)statusEl.textContent='压缩中... ('+loaded+'/'+total+')';if(loaded===total){{renderAddImageStrip();if(statusEl)statusEl.textContent=''}}}})}});input.value=''}}
