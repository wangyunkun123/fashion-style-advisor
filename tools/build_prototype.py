...
function sendOutfit(){{var inp=document.getElementById('today-input');var msg=inp.value.trim()||'推荐穿搭';inp.value='';showLoading(msg);fetch('/api/chat',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{message:msg}})}}).then(r=>r.json()).then(d=>{{if(d.task_id){{pollTask(d.task_id,1)}}else{{hideLoading();inp.placeholder=d.result||'已发送';setTimeout(function(){{loadTodayOutfit()}},2000)}}}}).catch(function(e){{hideLoading();inp.placeholder='网络错误: '+e.message}})}}
function pollTask(tid,n){{fetch('/api/task/'+tid).then(r=>r.json()).then(function(d){{if(d.status==='done'){{updateLoading('✅ 完成! 加载新穿搭...');setTimeout(function(){{hideLoading();loadTodayOutfit()}},1500)}}else if(d.status==='error'){{updateLoading('❌ '+(d.message||'生成失败'));setTimeout(hideLoading,3000)}}else{{var msgs=['🤖 AI分析穿搭方案中...','🎨 Seedream生成效果图...','🖼️ 排版合成中...','📤 推送同步中...'];var idx=Math.min(n-1,msgs.length-1);if(d.message)updateLoading(d.message);else updateLoading(msgs[idx]);setTimeout(function(){{pollTask(tid,n+1)}},3000)}}}}).catch(function(){{setTimeout(function(){{pollTask(tid,n+1)}},3000)}})}}
...
// Auto-load today's outfit on page open
function loadTodayOutfit(){{fetch('/api/today').then(r=>r.json()).then(function(d){{if(d&&!d.empty){{var hero=document.querySelector('.hero-card');if(!hero)return;var img=hero.querySelector('.hero-img img');if(img&&d.img)img.src=d.img;var style=hero.querySelector('.hero-style');if(style&&d.style)style.textContent=d.style;var items=hero.querySelector('.item-list')||hero.querySelector('.item-grid');if(items&&d.items){{var h='';var icons={{'TS':'tshirt','LS':'tshirt','SHIRT':'shirt','TANK':'tank','JK':'jacket','PT':'pants','SH':'shorts','SHOE':'shoe','HAT':'hat','BAG':'bag','SOCK':'sock','SUN':'sun','ACC':'acc'}};d.items.forEach(function(it){{var p=it.id.split('-')[0];h+='<div class=\"item-row\"><span class=\"item-emoji\"></span><span class=\"item-cat\"></span><span class=\"item-id\">'+it.id+'</span><span class=\"item-name\">'+it.name.substring(0,20)+'</span></div>'}});items.innerHTML=h}}}}}});setTimeout(function(){{location.reload()}},5000)}}
loadTodayOutfit();
</script>
</body></html>'''
