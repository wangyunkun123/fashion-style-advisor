...
.cover .tag.trend-popular{background:#fff0e5;color:#E07B39;font-weight:600}
.cover .tag.trend-classic{background:#eaf0f6;color:#2C3E50;font-weight:600}
.cover .tag.trend-niche{background:#f3edf9;color:#7B4FBF;font-weight:600}
...
def generate_one(style_id, dry_run=False):
    ...
    # 趋势分类标签（最前面，彩色突出）
    trend_cat = info.get('trend_category', '')
    TREND_ZH = {'popular_trend': '🔥 流行趋势', 'classic': '🏛️ 经典风格', 'niche': '🎭 小众领域'}
    TREND_CLASS = {'popular_trend': 'trend-popular', 'classic': 'trend-classic', 'niche': 'trend-niche'}
    trend_tags_html = ''
    if trend_cat in TREND_ZH:
        trend_tags_html = f'<span class="tag {TREND_CLASS.get(trend_cat, "")}">{TREND_ZH[trend_cat]}</span>'
    ...
    tags.extend(extra)
    tags_html = trend_tags_html + ''.join(f'<span class="tag">{t}</span>' for t in tags[:12])
