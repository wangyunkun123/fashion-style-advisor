...
function pinToHome(outfitId){{fetch('/api/pin',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{outfit_id:outfitId}})}}).then(function(r){{return r.json()}}).then(function(d){{if(d.status==='ok'){{showToast('📌 已放回主页，刷新后生效','#1e3a5f')}}}}).catch(function(){{showToast('网络错误','#c4523c')}})}}
</script>
</script>
</body></html>'''
...
# If an outfit is pinned, prefer it (find it in scan results)
if pinned_id and today:
    all_outfits = scan_outfits(limit=100)
    for o in all_outfits:
        if o['dir'] == pinned_id:
            today = [o] + today  # prepend: pinned outfit becomes hero
            break
if today:
    ho = today[0]
